class Snippet < ActiveRecord::Base
    ...
    def fetch(word)
        ...
        uri = URI.parse("https://www.googleapis.com/books/v1/volumes?q=#{word}&libraryRestrict=my-library&key=#{ENV['GOOGLE_PUBLIC_KEY']}&access_token=#{@@access_token}")
        ...
        self.parse_response(JSON.parse(response.body), word.downcase)
    end
    ...
    def parse_response(response, search_word)
    ...
    def fetch_list
        ...
        word_array[101..150].each do |word|
            self.fetch(word)
        end
    end
end
